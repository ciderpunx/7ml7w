...
Elm.Main.make = function (_elm) {
   ...
   var cycle = function (slides) {
      return A2($Basics._op["++"],
      $List.tail(slides),
      _L.fromArray([$List.head(slides)]));
   };
   var getNextSlide = function (slides) {
      return $List.head(slides);
   };
   var nextPan = function (s) {
      return A2($Random.generate,
      A2($Random.$float,-1,1),
      s);
   };
   var nextZoom = function (s) {
      return A2($Random.generate,
      A2($Random.$float,0,1),
      s);
   };
   var seed = A2($Signal._op["<~"],
   function (_v0) {
      return function () {
         switch (_v0.ctor)
         {case "_Tuple2":
            return $Random.initialSeed($Basics.round(_v0._0));}
         _U.badCase($moduleName,
         "on line 250, column 21 to 43");
      }();
   },
   $Time.timestamp($Signal.constant({ctor: "_Tuple0"})));
   var renderSlidePic = function (s) {
      return $Graphics$Collage.scale(s.zoom)($Graphics$Collage.move({ctor: "_Tuple2"
                                                                    ,_0: s.x
                                                                    ,_1: s.y})($Graphics$Collage.toForm($Graphics$Element.opacity(s.opacity)(s.pic))));
   };
   var delta = $AnimationFrame.frame;
   var Slide = F8(function (a,
   b,
   c,
   d,
   e,
   ...
   var initialZoom = 0.88;
   var secsPerSlide = 4;
   var fadeInSpeed = 0.5;
   var fadeInTime = 2;
   var zoom = 0.8;
   var zoomCalc = function (s) {
   ...
   var getRandomSlideParams = function (seed) {
      return function () {
         var z = nextZoom(seed);
         var x = nextPan($Basics.snd(z));
         var y = nextPan($Basics.snd(x));
         var iz = newInitZoom($Basics.snd(y));
         return {ctor: "_Tuple4"
                ,_0: x
                ,_1: y
                ,_2: z
   ...
   var nextSlide = F2(function (w,
   s) {
      return function () {
         var cur = s.nextSlide;
         var seed$ = s.rndReady ? s.seed : w.seed;
         var $ = getRandomSlideParams(seed$),
         x = $._0,
         y = $._1,
         z = $._2,
         iz = $._3;
         var slides$ = cycle(s.slides);
         var nxt = getNextSlide(slides$);
         return _U.replace([["curSlide"
                            ,cur]
                           ,["nextSlide"
                            ,_U.replace([["x",0]
                                        ,["xv",$Basics.fst(x) / 5]
   ...
   var panZoomFade = function (s) {
      return zoomCalc(panY(s.y)(A2(panX,
      s.x,
      s)));
   };
   var show = F2(function (w,s) {
      return function () {
         var nxt = s.nextSlide;
         var cur = s.curSlide;
         ...
         wdth - (wdth / 4 | 0),
         hght - (hght / 4 | 0),
         _L.fromArray([renderSlidePic(cur)
                      ,renderSlidePic(nxt)
                      ,$Graphics$Collage.move({ctor: "_Tuple2"
                                              ,_0: 0
                                              ,_1: -120})($Graphics$Collage.toForm($Graphics$Element.color($Color.green)($Text.asText(cur.xv))))]))));
      }();
   });
   var inBounds = F4(function (n,
   c,
   t,
   w) {
      return _U.cmp(c.x,
      $Basics.toFloat(w.w) - $Basics.toFloat(wdth)) < 0 && (_U.cmp(c.zoom,
      maxZoom) < 0 && (_U.cmp(n.opacity,
      1) < 0 && _U.cmp(t,
      secsPerSlide) < 1));
   });
   var stepShow = F2(function (w,
   s) {
      return function () {
         var time = s.frames / 60 | 0;
         var nxt = panZoomFade(s.nextSlide);
         var fadeIn = _U.cmp(time,
         fadeInTime) > 0 ? nxt.opacity + fadeInSpeed / 100 : nxt.opacity;
         var fadeOut = 1 - 2 * fadeIn;
         var cur = panZoomFade(s.curSlide);
         var advance = A4(inBounds,
         nxt,
         cur,
         time,
         w);
         return advance && _U.eq(s.status,
         Playing) ? _U.replace([["curSlide"
                                ,_U.replace([["opacity"
                                             ,fadeOut]],
                                cur)]
                               ,["nextSlide"
   ...
   var defSlideImg = "henneys_vintage.jpg";
   var filePath = "/graphics/reviews/";
   var mkImgStr = function (s) {
      return A2($Basics._op["++"],
      filePath,
      s);
   };
   var mkImg = function (str) {
   ...
   var defSlide = {_: {}
                  ...
                  ,zoom: initialZoom
                  ,zv: 1.0e-4};
   var mkSlide = function (str) {
      return _U.replace([["pic"
                         ,mkImg(str)]],
      defSlide);
   };
   var mkSlides = $List.map(mkSlide);
   var slideImages = _L.fromArray(["ciderniks-kingston-black.jpg"
                                  ...
                                  ,"crossmans-cider.jpg"]);
   var defSlideshow = function () {
      var nxt = defSlide;
      var cur = defSlide;
      return {_: {}
             ,curSlide: cur
             ,frames: 0
             ,nextSlide: _U.replace([["xv"
                                     ,4.0e-2]
                                    ,["yv",-2.0e-2]
                                    ,["zv",2.0e-3]],
             nxt)
   ...
   _elm.Main.values = {_op: _op
                      ...
                      ,stepShow: stepShow
                      ,nextSlide: nextSlide
                      ,renderSlidePic: renderSlidePic
                      ,inBounds: inBounds
                      ,panZoomFade: panZoomFade
                      ,zoomCalc: zoomCalc
                      ,panX: panX
                      ,panY: panY
                      ,mkSlides: mkSlides
                      ...
                      ,mkImgStr: mkImgStr
                      ,seed: seed
                      ,getRandomSlideParams: getRandomSlideParams
                      ,nextZoom: nextZoom
                      ,newInitZoom: newInitZoom
                      ,nextPan: nextPan
